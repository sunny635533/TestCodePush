...
#import "HongSiRN-Swift.h"

//#import "BigBigChannel-Swift.h"
...
RCT_EXPORT_METHOD(getAllItems:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject){
    NSDictionary *dicts = [[NSUserDefaults standardUserDefaults] dictionaryRepresentation];
    resolve(dicts);
}

RCT_EXPORT_METHOD(getLoginInfos:(NSDictionary *)array options:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject){

    NSMutableDictionary *mutableArray = [NSMutableDictionary new];
    NSArray *keys = [array allKeys];
    for(NSString* nativeKey in keys){
        NSString* nativeValueKey = array[nativeKey];
        NSString *value = [[NSUserDefaults standardUserDefaults] valueForKey:nativeValueKey];
        // your code here
        NSLog(@"value: %@ forKey: %@",value,nativeValueKey);
        [mutableArray setObject:(value?: @"") forKey:nativeKey];
    }
