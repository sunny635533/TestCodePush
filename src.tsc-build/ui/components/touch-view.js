...
let ChooseGroup = class ChooseGroup extends React.Component {
    ...
    render() {
        return (React.createElement(react_native_1.View, { style: this.props.style }, this.props.beanList.map((bean, i) => {
            ...
            return (React.createElement(TouchImageWithText, { key: i, index: actualIndex, imageUri: bean.imageUri, text: bean.text, onPress: () => this.onPress(actualIndex, bean.onPress), imageStyle: imageStyle, textStyle: textStyle, touchStyle: touchStyle }));
        })));
    }
    onPress(index, press) {
...
function TouchImageWithText(ps) {
    return React.createElement(react_native_1.TouchableOpacity, { activeOpacity: 1, style: [styles.center, ps.touchStyle], onPress: () => ps.onPress(ps.index) },
